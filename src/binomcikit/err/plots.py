...
def _err_plot(df, title):
    ...
    if "method" not in df.columns:
        df = df.copy()
        df["method"] = "Method"
    va = df[["method", "delalp", "Fail_Pass"]].rename(columns={"delalp": "value"})
    va["mark"] = "Increase in nominal error"
    vt = df[["method", "theta", "Fail_Pass"]].rename(columns={"theta": "value"})
...
def _make_base_plot(fn, label):
    def _plot(n, alp, phi, f):
        return _err_plot(
            fn(n, alp, phi, f), f"Error, long term power and pass/fail - {label} method"
        )
...
def ploterrex(n, alp, phi, f, e):
    ...
    return _err_plot(
        base_all.errex(n, alp, phi, f, e), "Error, long term power and pass/fail - Exact method"
    )
...
def ploterrall(n, alp, phi, f):
    ...
    return _err_plot(
        base_all.errall(n, alp, phi, f), "Error, long term power and pass/fail - all base methods"
    )
...
def _make_adj_plot(fn, label):
    def _plot(n, alp, h, phi, f):
        return _err_plot(
            fn(n, alp, h, phi, f), f"Error, long term power and pass/fail - adjusted {label} method"
        )
...
def _make_cc_plot(fn, label):
    def _plot(n, alp, phi, c, f):
        return _err_plot(
            fn(n, alp, phi, c, f), f"Error, long term power and pass/fail - CC {label} method"
        )
...
def ploterrcall(n, alp, phi, c, f):
    ...
    return _err_plot(
        cc_all.errcall(n, alp, phi, c, f), "Error, long term power and pass/fail - all CC methods"
    )
...
def ploterrgen(n, LL, UL, alp, phi, f):
    ...
    return _err_plot(
        errgen(n, LL, UL, alp, phi, f), "Error, long term power and pass/fail (given limits)"
    )
...
def ploterrba(n, alp, phi, f, a, b):
    ...
    return _err_plot(
        errba(n, alp, phi, f, a, b), "Error, long term power and pass/fail - Bayesian method"
    )
