...
def _to_long(df):
    ...
    long = df.melt(
        id_vars=id_vars, value_vars=["pconf", "pbias"], var_name="Heading", value_name="Value"
    )
...
def plotpcopbiex(n, alp, e):
    ...
    return _pconf_plot(base_all.pcopbiex(n, alp, e), "p-Confidence & p-Bias - Exact method")


def plotpcopbiall(n, alp):
    """p-confidence & p-bias plot for all six base methods (R PlotpCOpBIAll)."""
    return _pconf_plot(base_all.pcopbiall(n, alp), "p-Confidence & p-Bias - all base methods")


# --- Adjusted method plots (R 412) -------------------------------------------
def _make_adj_plot(fn, label):
    def _plot(n, alp, h):
        return _pconf_plot(fn(n, alp, h), f"p-Confidence & p-Bias - adjusted {label} method")
...
def plotpcopbiaall(n, alp, h):
    ...
    return _pconf_plot(
        adj_all.pcopbiaall(n, alp, h), "p-Confidence & p-Bias - all adjusted methods"
    )
...
def _make_cc_plot(fn, label):
    def _plot(n, alp, c):
        return _pconf_plot(
            fn(n, alp, c), f"p-Confidence & p-Bias - continuity-corrected {label} method"
        )
...
def plotpcopbicall(n, alp, c):
    ...
    return _pconf_plot(
        cc_all.pcopbicall(n, alp, c), "p-Confidence & p-Bias - all continuity-corrected methods"
    )
...
def plotpcopbigen(n, LL, UL):
    ...
    return _pconf_plot(pcopbigen(n, LL, UL), "p-Confidence & p-Bias (given limits)")
...
def plotpcopbiba(n, alp, a1, a2):
    ...
    q = ba[["x1", "pconfQ", "pbiasQ"]].rename(columns={"pconfQ": "pconf", "pbiasQ": "pbias"})
    q["method"] = "Quantile"
    h = ba[["x1", "pconfH", "pbiasH"]].rename(columns={"pconfH": "pconf", "pbiasH": "pbias"})
    h["method"] = "HPD"
    return _pconf_plot(
        pd.concat([q, h], ignore_index=True), "p-Confidence & p-Bias - Bayesian method"
    )
