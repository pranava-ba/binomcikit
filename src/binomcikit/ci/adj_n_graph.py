...
def plotciawd(n, alp, h):
    ...
    ldf = pd.concat(
        [
            ll[["ID", "LowerLimit"]]
            .assign(Abberation="Lower")
            .rename(columns={"LowerLimit": "Value"}),
            ul[["ID", "UpperLimit"]]
            .assign(Abberation="Upper")
            .rename(columns={"UpperLimit": "Value"}),
            zl[["ID", "LowerLimit"]]
            .assign(Abberation="ZWI")
            .rename(columns={"LowerLimit": "Value"}),
        ]
    )

    plot = (
        ggplot(ss1, aes(x="UpperLimit", y="ID"))
    ...
    if not ldf.empty:
        plot += geom_point(
            data=ldf, mapping=aes(x="Value", y="ID", shape="Abberation", color="Abberation"), size=4
        )
...
def plotciaas(n, alp, h):
    ...
    plot = (
        ...
        + labs(
            ...
            y="x values",
            title="Confidence interval for Adjusted ArcSine Method",
        )
    )
...
def plotcialt(n, alp, h):
    ...
    if not ldf.empty:
        plot = (
            ...
            + geom_point(
                ldf,
                aes(x="Value", y="ID", group="Abberation", shape="Abberation"),
                size=4,
                fill="red",
            )
